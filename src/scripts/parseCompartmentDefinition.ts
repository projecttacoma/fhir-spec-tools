...
const compartmentDefPath = path.resolve(
  path.join(__dirname, "../fhir/compartmentdefinition-patient.json")
);
const attrOutputPath = path.resolve(
  path.join(__dirname, "../data/patient-attribute-paths.json")
);
const paramOutputPath = path.resolve(
  path.join(__dirname, "../data/patient-search-parameters.json")
);
...
async function parse(compartmentJson: string) {
  const compartmentDefinition = (await JSON.parse(
    compartmentJson
  )) as fhir4.CompartmentDefinition;
  ...
  compartmentDefinition.resource?.forEach((resourceObj) => {
    if (resourceObj.param) {
      ...
      const searchParameterList = getSearchParameters(
        resourceObj.code,
        "4_0_0"
      ).filter((objs: any) => resourceObj.param?.includes(objs.name));
      searchParameterList.forEach((obj: any) => {
        // retrieve xpath and remove resource type from beginning
        attrResults[resourceObj.code].push(
          obj.xpath.substr(obj.xpath.indexOf(".") + 1)
        );
      });
    }
  });
